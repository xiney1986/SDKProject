...
public class ErlArray:ErlType
{
	...
	public string getListArray ()
	{
		...
		if (len == 2 && (_value [0] as ErlByte) != null && (_value [0] as ErlByte).Value == 1 &&
			...
			str = "[]";
		} else if (len > 0 && len % 2 == 0) {// kv结构解析
			str = "{";
			for (int i=0; i<len-1; i+=2) {
				if (_value [i] as ErlAtom != null || _value [i] as ErlList != null || _value [i] as ErlByteArray != null || _value [i] as ErlString != null) {
					if (_value [i + 1] != null)
	...
	public static ErlArray strToErlArr (string str) {
		...
		List<object> jsonList = MiniJSON.Json.Deserialize (str) as List<object>;
