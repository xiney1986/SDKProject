...
public class ActiveOnlineTime:ActiveTime
{
	//相对开服时间的相对时间
	private int sTime;
	//持续时间
	...
	public override void computeTime (int time)
	{
		...
		if (duration == 0) {
			...
			if(time >= endTime){
				startTime += duration + cycleTime;
				endTime += duration + cycleTime;
			}
		}
	}
    ...
    public override string ToString()
    {
        ...
        {
            sb.Append("开始时间:");
            sb.Append("开服"+(timeSample.mainTimeInfo[0] < 0?"前":"后")+":"+GetTime(timeSample.mainTimeInfo[0]));
            sb.Append("\n");
            sb.Append("持续时间:");
