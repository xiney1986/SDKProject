...
using System;
/// <summary>
/// 天梯属性加成更新的处理 是服务器主动推送过来的
/// </summary>
public class LaddersAttrEffectUpdateFPort : BaseFPort
{
	...
	public override void read (ErlKVMessage message)
	{
		...
		for(int i=0,length=effectList.Value.Length;i<length;i++)
		{
			...
			{
				LaddersManagement.Instance.TitleAttrEffect.updateInfoByServer(effectItem.Value[1] as ErlArray);
			}else if(id==id_ladder_medal)
			{
				LaddersManagement.Instance.MedalAttrEffect.updateInfoByServer(effectItem.Value[1] as ErlArray);
			}
		}
	}
}
