 ...
 * 队伍编辑窗口
 * */ 
public class  TeamViewInMissionWindow1 : WindowBase
{
...
//    public const int FROM_TOWER = 6;//爬塔
	...
	{ 
		base.begin ();
		ArmyManager.Instance.teamEditInMissonWin = true;
       // MaskWindow.UnlockUI();
...
//			}
		...
		if (GuideManager.Instance.isEqualStep (GuideGlobal.SPECIALSID3) || GuideManager.Instance.isEqualStep (GuideGlobal.SPECIALSID33)
			...
			for (int i = 0; i < teamForRole.Length; i++) {
				teamForRole [i].GetComponent<TeamEditDragDropItemInMissionWin> ().enabled = false;
				teamForRole [i].GetComponent<TeamEditDragDropItemInMissionWin> ().cloneOnDrag = true;
			}
			for (int i = 0; i < teamSubRole.Length; i++) {
				teamSubRole [i].GetComponent<TeamEditDragDropItemInMissionWin> ().enabled = false;
				teamSubRole [i].GetComponent<TeamEditDragDropItemInMissionWin> ().cloneOnDrag = true;
			}
		} else {
...
//			saveButton.GetComponent<BoxCollider> ().enabled = true;
			...
			for (int i = 0; i < teamForRole.Length; i++) {
				teamForRole [i].GetComponent<TeamEditDragDropItemInMissionWin> ().enabled = true;
				teamForRole [i].GetComponent<TeamEditDragDropItemInMissionWin> ().cloneOnDrag = false;
			}
			for (int i = 0; i < teamSubRole.Length; i++) {
				teamSubRole [i].GetComponent<TeamEditDragDropItemInMissionWin> ().enabled = true;
				teamSubRole [i].GetComponent<TeamEditDragDropItemInMissionWin> ().cloneOnDrag = false;
			}
		}
		openEmptyForLevel ();
