 ...
 * @author longlingquan
 * */
public class TowerBuyWindow : WindowBase
{
...
//			callback (msg);
	...
	public void calculateTotal ()
	{
		...
		//这里添加多样性 
            ...
            int allMoney = 0;
            if (buyStruct.goodsType == "BoxTimes") {//神魔宝箱开启次数
                msg.msgNum = now;
                for (int i = 0; i < now; i++) {
                    if (buyStruct.unitPrice + i >= CommandConfigManager.Instance.getTowerBuyCost().Length)
                        allMoney += CommandConfigManager.Instance.getTowerBuyCost()[CommandConfigManager.Instance.getTowerBuyCost().Length - 1];
                    else
                        allMoney += CommandConfigManager.Instance.getTowerBuyCost()[buyStruct.unitPrice + i];                
                }
                totalMoney.text = allMoney.ToString();
            } else {
