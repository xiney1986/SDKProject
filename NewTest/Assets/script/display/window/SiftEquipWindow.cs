...
public class SiftEquipWindow : WindowBase
{
	public UIToggle[] equipjob_box;
	public UIToggle[] quality_box;
	public UIToggle[] part_box;
	...
	private void initCheckLabel ()
	{
		...
		part_box [0].GetComponent<SiftItemModule> ().text.text = LanguageConfigManager.Instance.getLanguage ("s0249");
		part_box [1].GetComponent<SiftItemModule> ().text.text = LanguageConfigManager.Instance.getLanguage ("s0250");
		part_box [2].GetComponent<SiftItemModule> ().text.text = LanguageConfigManager.Instance.getLanguage ("s0251");
		part_box [3].GetComponent<SiftItemModule> ().text.text = LanguageConfigManager.Instance.getLanguage ("s0252");
		part_box [4].GetComponent<SiftItemModule> ().text.text = LanguageConfigManager.Instance.getLanguage ("s0253");
...
//			sim.text.color = Colors.BUTTON_TEXT_NROMAL;
	...
	//初始化部位条件
	public void initPartCondition (int part)
	{
		//以下是限定部件的选择
		if (part == 0)
			return;
		...
		for (int i = 0; i < part_box.Length; i++) {
			
			//if (cutString (part_box [i].name) == "part") {
			if (toInt (part_box [i].name) != part) {
				disableCheckState (part_box [i]);
			} 

			part_box [i].GetComponent<BoxCollider> ().enabled = false;
			//	}
		...
		if (sc != null && sc.siftConditionArr != null && sc.siftConditionArr.Length > 0) {
			for (int i=0; i < sc.siftConditionArr.Length; i++) {
				...
				case SortType.EQUIP_PART:
					changeBox (part_box, condition.getConditions ().ToArray (), "part");
					break;
	...
	private void clearBox ()
	{
		...
		for (int i=0; i<part_box.Length; i++) {
			part_box [i].value = false;
			enableCheckState (part_box [i]);
		}
	...
	private  Condition[] getAllChecked ()
	{
		...
		for (int i = 0; i < part_box.Length; i++) {
			if (part_box [i].value) {
				cons [2].addCondition (toInt (part_box [i].name));
			}
		}
	...
	private  Condition getAllChecked (int type)
	{
		...
		case SortType.QUALITY:
			...
			break;
		case SortType.EQUIP_PART:
			con = new Condition (SortType.EQUIP_PART);
			for (int i = 0; i < part_box.Length; i++) {
				if (part_box [i].value) {
					con.addCondition (toInt (part_box [i].name));
				}
			}
			break;
		}
