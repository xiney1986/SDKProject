...
/// 猎杀星魂容器
/// </summary>
public class SoulHuntContent : MonoBase {
...
//		(FPortManager.Instance.getFPort ("StarSoulHuntFPort") as StarSoulHuntFPort).autoHuntAccess (currentTapIndex, doHuntFinshedCall);
	...
	public void setIntegralRank(int _integral,int _rank){
		integral = _integral;
		rank = _rank;
		integralLabel.text = integral.ToString();
		if (_integral == 0||_rank < 0||_rank==0) {
			rankValueLabel.text = LanguageConfigManager.Instance.getLanguage ("s0560");
		}
		else {
			rankValueLabel.text = _rank.ToString ();
		}
        if (showPrizes.Count > 0)
        {
            UiManager.Instance.openDialogWindow<AllAwardViewWindow>((win) =>
	...
	public void showIntegralRankLabel()
	{
		integralLabel.text = integral.ToString();
		if (integral == 0||rank < 0||rank==0) {
			...
			rankValueLabel.text = rank.ToString ();
		}
	}
}