...
public class LastBattleRankItemView : MonoBase
{
	/** 各Label */
	...
	[HideInInspector]
	public LastBattleRank data;
	[HideInInspector]
	public int index;

	public void init (LastBattleRank data,int index)
	{
		this.data = data;
		this.index = index;
	...
	void setText (LastBattleRank data,int index)
	{
		int pos=0;
		texts[pos++].text = (index+1).ToString();
		texts[pos++].text = "["+data.serverName+"]"+data.name;
		texts[pos++].text = data.score.ToString();
	}
