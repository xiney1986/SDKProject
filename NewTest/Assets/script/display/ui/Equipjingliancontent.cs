...
/// 共鸣--装备精炼容器
/// </summary>
public class Equipjingliancontent : MonoBase 
{
    ...
    public UILabel lvInfoTile;//精炼等级标题
    ...
    private void updateEqItem()
    {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            if (tempList == null || tempList.Count <= 0) {
                eif.initRefine(fatherWindow, null, i + 1, showTypeNum);
            } else {
                bool flag = false;
                for (int k = 0; k < tempList.Count; k++) {
                    ...
                    if (tmp == i + 1) {
                        flag = true;
                        eif.initRefine(fatherWindow, tempList[k], 0, showTypeNum);
                        break;
                    }
                }
                if (!flag) eif.initRefine(fatherWindow, null, i + 1, showTypeNum);
            }
        }
    }
    /// <summary>
